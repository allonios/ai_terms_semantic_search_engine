...
from owlready2.prop import DataProperty, DataPropertyClass, ObjectPropertyClass
...
def FLS(onto1, onto2):
    ...
    return (iden_conc_label + iden_prop_label) / max(
...
def get_number_of_common_nonl_classes(onto1, onto2):
    ...
    _classes_1 = list(
        map(
            lambda c: f"{len(list(c.subclasses()))}-{get_length_from_root(onto1, c)}",
            nonl_classs_1,
        )
    )
    ...
    _classes_2 = list(
        map(
            lambda c: f"{len(list(c.subclasses()))}-{get_length_from_root(onto2, c)}",
            nonl_classs_2,
        )
    )

    return (
        len(list(filter(lambda x: x in _classes_1, _classes_2))),
        len(nonl_classs_1),
        len(nonl_classs_2),
    )
...
def get_number_of_common_nonl_properties(onto1, onto2):
    ...
    _properties_1 = list(
        map(
            lambda p: f"{len(list(p.subclasses()))}-{get_length_from_root(onto1, p)}",
            nonl_property_1,
        )
    )
    ...
    _properties_2 = list(
        map(
            lambda p: f"{len(list(p.subclasses()))}-{get_length_from_root(onto2, p)}",
            nonl_property_2,
        )
    )

    return (
        len(list(filter(lambda x: x in _properties_1, _properties_2))),
        len(nonl_property_1),
        len(nonl_property_2),
    )
...
def FSS(onto1, onto2):
    (
        ...
        nonl_classs_1_len,
        nonl_classes_2_len,
    ) = get_number_of_common_nonl_classes(onto1, onto2)

    (
        common_nonl_properties,
        nonl_properties_1_len,
        nonl_properties_2_len,
    ) = get_number_of_common_nonl_properties(onto1, onto2)
...
def calc_similarity(name1, name2):
    ...
    simliarity_matrix = fast_calc_similarity(name1_tokens, name2_tokens)
...
def calculate_concept_matrix(concept, use_hier_features=True):
    ...
    if use_hier_features:
        results += list(
            reduce(lambda acc, c: acc + Meta(c), Hier(concept), [])
        )
...
def calculate_property_matrix(property, use_hier_features=True):
    ...
    if use_hier_features:
        results += list(
            reduce(lambda acc, p: acc + Meta(p), Hier(property), [])
        )
...
def sim_vec(
    onto1, onto2, e1, e2, use_hier_features=True, add_more_features=False
):
    if isinstance(e1, ThingClass):
        e1_matrix = calculate_concept_matrix(e1, use_hier_features)
    elif (
        isinstance(e1, ObjectPropertyClass)
        or isinstance(e1, DataPropertyClass)
        or isinstance(e1, AnnotationPropertyClass)
    ):
    ...
    if isinstance(e2, ThingClass):
        ...
        isinstance(e2, ObjectPropertyClass)
        or isinstance(e2, DataPropertyClass)
        or isinstance(e2, AnnotationPropertyClass)
    ):
...
def DLG_O_for_property(p: DataPropertyClass, onto):
    ...
    triples = triples + list(
        map(lambda sp: (p, labels["HasSubProperty"], sp), Hier(p))
    )
    triples = triples + list(
        map(lambda sp: (p, labels["HasRange"], sp), Rang(p))
    )
    triples = triples + list(
        map(lambda sc: (p, labels["HasPropertySibling"], sc), Sibl(p, onto))
    )
    return triples
...
def DLG_O_for_concept(c: ThingClass, onto):
    triples = []
    triples = triples + list(
        map(lambda sc: (c, labels["HasSubConcept"], sc), Hier(c))
    )
    triples = triples + list(
        map(lambda p: (c, labels["HasProperty"], p), Rest(c))
    )
    triples = triples + list(
        map(lambda sc: (c, labels["HasConceptSibling"], sc), Sibl(c, onto))
    )
    return triples
...
def DLG_O(onto):
    triples = []
    for e in NO_O(onto):
        if isinstance(e, ObjectPropertyClass) or isinstance(
            e, DataPropertyClass
        ):
    ...
    for triple_1 in triples_1:
        for triple_2 in triples_2:
            if triple_1[1] == triple_2[1]:
                pcg.append(
                    (
                        (triple_1[0], triple_2[0]),
                        triple_1[1],
                        (triple_1[2], triple_2[2]),
                    )
                )
...
def SPG_O(onto_1, onto_2):
    ...
    for triples in pcg:
        ...
        if not left:
            ...
            grouped_by_left[triples[0]] = {
                "sigma": names_sim,
                "sigma_0": names_sim,
                "connections": [],
            }
        grouped_by_left[triples[0]]["connections"].append(
            (triples[1], triples[2])
        )
...
def calc_phi(rel, spg, propagation_only_on_CP=False):
    ...
    for rel_type, out_rel in rel["connections"]:
        if (
            propagation_only_on_CP
            and rel_type != labels["HasRange"]
            and rel_type != labels["HasProperty"]
        ):
            continue
        result += (1 / len(rel["connections"])) * spg.get(
            out_rel, {"sigma": 0}
        )["sigma"]
...
def update_SPG_O(spg, propagation_only_on_CP=False):
    ...
    for _, value in spg.items():
        value["sigma"] = (
            value["sigma_0"]
            + value["sigma"]
            + calc_phi(value, spg, propagation_only_on_CP)
        )
...
def get_structural_matched_entities(
    onto1, onto2, threshold=0.8, iterations=1000
):
    result = {}
...
def sim_combination(onto1, onto2, e1, e2):
    ...
    svec = sim_vec(
        onto1, onto2, e1, e2, use_hier_features, add_more_features_to_VD
    )
...
def get_hybred_matched_entities(onto1, onto2):
    ...
    if fls != 0:
        textual_matchings = defaultdict(
            int, get_textual_matched_entities(onto1, onto2)
        )

    if fss != 0:
        structural_matchings = defaultdict(
            int, get_structural_matched_entities(onto1, onto2)
        )

    all_keys = set(
        list(textual_matchings.keys()) + list(structural_matchings.keys())
    )
    ...
    for e1, e2 in all_keys:
        sim = (
            textual_matchings[(e1, e2)] * fls
            + structural_matchings[(e1, e2)] * fss
        )
        if max_1to2[e1] < sim:
            max_1to2[e1] = sim
            matchings_1to2[e1] = e2
